		...
		if(Pagedp2pProductRecordList!=null){
			...
			for(Sales sales:p2pProductRecordList){
				...
				if(p2pCustomerNo!=0){
					...
					if(p2pCustomer!=null){
						name = p2pCustomer.getCellphone().trim();
					}
				}else{
					int customerNo = sales.getCustomerNo();
					CustomerPersonal customer = CustomerPersonalModel.getInfo(customerNo);
					if(customer!=null){
						if(!StringHelper.isNullOrEmpty(customer.getCellphone1())){
							name = customer.getCellphone1().trim();
						}else if(!StringHelper.isNullOrEmpty(customer.getCellphone2())){
							name = customer.getCellphone2().trim();
						}else{
							name = customer.getPhone().trim();
						}
					}
				}
