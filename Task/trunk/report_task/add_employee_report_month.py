...
def __main():
	try:
		...
		for employee in employees:
			...
			add_b,add_c,add_d,add_employee_count,employee_count,type,emp_name,emp_status,year,month)
			values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s);
			'''
			cursor_report.execute(sql_insert_add_employee_report,(employee['user_no'],dept_no,company_no,\
				add_a,add_b,add_c,add_d,add_employee_count,customer_count,report_type,emp_name,emp_status,year,month));
...
def __getAddCountByLevelAndAgentNo(level,agent_no):
	try:
		...
		sql_select_add_count = '''
			...
			PERIOD_DIFF( date_format( now( ) , '%Y%m' ) , date_format( in_time, '%Y%m' ) ) =1
		'''
