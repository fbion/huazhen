...
public class AjaxProductAction extends JqGridBaseAction<Product> {
    ...
    public String execute() throws Exception {
        ...
        productCondition.setPageIndex(this.getPageIndex());
//        productCondition.setPageSize(this.rows);
//        productCondition.setPageIndex(this.page);

        if (!StringHelper.isNullOrEmpty(this.byName)) {
            productCondition.setName(this.byName);
        }

        if (StringHelper.isNullOrEmpty(this.byStatus)) {
            productCondition.setStatus(0);
        } else {
            productCondition.setStatus(Integer.parseInt(this.byStatus));
        }
        if (StringHelper.isNullOrEmpty(this.byType)) {
            productCondition.setType(0);
        } else {
            productCondition.setType(Integer.parseInt(this.byType));
        }
        
        if(StringHelper.isNullOrEmpty(getIsTest())){
        	productCondition.setIsTest((byte)0);
        }else {
        	productCondition.setIsTest(Byte.parseByte(getIsTest()));
        }
        ...
        this.setResultList(productPagedList.getResultList());
        this.setRows(productPagedList.getResultList());
        this.setPageCount(productPagedList.getPagingInfo().getPageCount());
        this.setPageIndex(productPagedList.getPagingInfo().getPageIndex());
        this.setRecordCount(productPagedList.getPagingInfo().getTotalCount());
...
//    public int addLetterForProductEstablish(String content){
//        Letter letter = new Letter();
//        letter.setContent(content);
//        letter.setRecipient(0);
//        letter.setImportantDegree(3);
//        letter.setType((byte)2);
//        letter.setSubject("产品成立公告");
//        letter.setInUserNo(10000);
//        return LetterModel.add(letter);
//    }
