var ManagePage = {
    ...
    GetInfo: function (id) {
        var url = $Url.BuildMarketUrl("/market/activityUsers/ajaxGetActivityUsers");
        $.ajax({
            type: "post",
            url: url,
            dataType: "json",
...
$(function () {
    $("#back").click(function () {
        //window.location.href = $Url.BuildMarketUrl("/market/activityUsers/list");
        window.close();
    });
    ...
    var activityUsersAdd = $("#activityUsersAdd").Validform({
        ...
        callback: function (form) {
            ...
            var url = $Url.BuildMarketUrl("/market/activityUsers/ajaxEditActivityUsers");
            var oper = "add";
            if (Number(PageVar.ID) != 0)
                oper = "edit";
            ...
            $.ajax({
                ...
                success: function (data, textStatus) {
                    if (data.errCode == "0000") {
                        window.location.href = $Url.BuildMarketUrl("/market/activityUsers/edit?id=" + data.errDesc);
                    } else {
                        $("#msg").text(data.errDesc);
                    }
                },
