            ...
            success: function (data, textStatus) {
                ...
                if ($("#edit").length > 0) {
                    $("#edit").click(function () {
                        ...
                        if(ElementVar.updateManagerNo!="none"){
		...
		InitGrid : function() {
			if ($String.Trim($("#gridTableFollow").html()) != "") {
				...
				$("#gridTableFollow").jqGrid({
					...
					mtype : 'GET',
					colNames: ["操作","id","产品","跟踪类型","跟踪的客户","本次联系人","联系人职位","跟踪类型","跟踪时间","下次跟踪时间","跟踪内容","跟踪结果","修改备注"],//,"产品类型"
					colModel: [
...
//								},
								...
								{
									name: "agentNo", index: "agentNo", width: 40, align: "left", formatter : "select",edittype : "select",editoptions : {size : 1,value : EnumList.GetEnumListToEdit("customerPerson",$Url.BuildCustomerUrl("/common/enumList.action"))},formoptions : {rowpos : 3,colpos : 2},sortable : false,	editable : true,hidden : true,editrules : {edithidden : true}
								},
								{
...
$(function () {
    ...
    var agentAdviserAdd = $("#agentAdviserAdd").Validform({
        tiptype: function (msg, o, cssctl) {
            var objtip = o.obj.siblings(".Validform_checktip");
            cssctl(objtip, o.type);
            objtip.text(msg);
        ...
        callback: function (form) {
            ...
            var url = $Url.BuildCustomerUrl("/customer/agentAdviser/ajaxEditAgentAdviser");
            var oper = "add";
            if (Number(PageVar.ID) != 0)
                oper = "edit";
            ...
            $.ajax({
                ...
                success: function (data, textStatus) {
                    if (data.errCode == "0000") {
                        window.location.href = $Url.BuildCustomerUrl("/customer/agentAdviser/detail?id=" + data.errDesc);
                    } else {
                        $("#msg").text(data.errDesc);
                    }
                },
