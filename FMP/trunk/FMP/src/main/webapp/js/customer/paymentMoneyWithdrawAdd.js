var ManagePage = {
    ...
    GetInfo: function (id) {
        var url = $Url.BuildCustomerUrl("/customer/paymentMoneyWithdraw/ajaxGetPaymentMoneyWithdraw");
        $.ajax({
            type: "post",
            url: url,
            dataType: "json",
...
var BindEnumList={
		...
		BindState:function(){
			EnumList.GetEnumListToSelect($("#state"),"paymentMoneyWithdrawState",$Url.BuildEmployeeUrl("/common/enumList.action"));
		}
	}
$(function () {
	BindEnumList.BindAll();
    $("#back").click(function () {
//        window.location.href = $Url.BuildCustomerUrl("/customer/paymentMoneyWithdraw/list");
        window.close();
    });
