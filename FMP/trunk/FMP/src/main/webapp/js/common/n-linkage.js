$.fn.linkage = function (options) {
    ...
    if (elements.length > 1 && elements.length == dataTypes.length) {
        ...
        for (i = 1; i < elements.length; i++) {
            ...
            $(elements[i]).linkageForJqGrid(op,arr);
        }
    }
};
...
$.fn.linkage.defaults = {
    elements: [],
    dataTypes: [],//["role",{1:"table1",2:"table2"}] ["table1","table2"]
    actionUrl: "",
    all: true
};

$.fn.linkageForJqGrid = function (options,arr) {
    ...
    if (opts.dataType instanceof Object) {
        ...
        $(opts.prev).change(function () {
        	Linkage.emptyAfterSelect($(this),arr);//lyh
...
var Linkage = {
    emptyAfterSelect: function(curObj,arr){//lyh
    	var num = 0;
