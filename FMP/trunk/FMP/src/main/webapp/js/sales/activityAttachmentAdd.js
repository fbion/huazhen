var ManagePage = {
    ...
    GetInfo: function (id) {
        var url = $Url.BuildSalesUrl("/sales/activityAttachment/ajaxGetActivityAttachment");
        $.ajax({
            type: "post",
            url: url,
            dataType: "json",
...
$(function () {
    $("#back").click(function () {
        window.location.href = $Url.BuildSalesUrl("/sales/activityAttachment/list");
    });
    ...
    var activityAttachmentAdd = $("#activityAttachmentAdd").Validform({
        ...
        callback: function (form) {
            ...
            var url = $Url.BuildSalesUrl("/sales/activityAttachment/ajaxEditActivityAttachment");
            var oper = "add";
            if (Number(PageVar.ID) != 0)
                oper = "edit";
            ...
            $.ajax({
                ...
                success: function (data, textStatus) {
                    if (data.errCode == "0000") {
                        window.location.href = $Url.BuildSalesUrl("/sales/activityAttachment/edit?id=" + data.errDesc);
                    } else {
                        $("#msg").text(data.errDesc);
                    }
                },
