var ManagePage = {
    ...
    GetInfo: function (id) {
        var url = $Url.BuildSalesUrl("/sales/applyCustomer/ajaxGetApplyCustomer");
        $.ajax({
            type: "post",
            url: url,
            dataType: "json",
...
$(function () {
    $("#back").click(function () {
        window.location.href = $Url.BuildSalesUrl("/sales/applyCustomer/list");
    });
    ...
    var applyCustomerAdd = $("#applyCustomerAdd").Validform({
        ...
        callback: function (form) {
            ...
            var url = $Url.BuildSalesUrl("/sales/applyCustomer/ajaxEditApplyCustomer");
            var oper = "add";
            if (Number(PageVar.ID) != 0)
                oper = "edit";
            ...
            $.ajax({
                ...
                success: function (data, textStatus) {
                    if (data.errCode == "0000") {
                        window.location.href = $Url.BuildSalesUrl("/sales/applyCustomer/edit?id=" + data.errDesc);
                    } else {
                        $("#msg").text(data.errDesc);
                    }
                },
