...
var ManagePage = {
	Resize:function(){
		$("#content_center").css({"min-height":"803px","height":$(".wrappContent").height()});
	},
...
var FileManage = {
    ...
    DeleteFile: function (o) {
        if (confirm("是否要删除当前文件")) {
            ...
            var url = $Url.BuildProductUrl("/employee/knowledgeAttachment/ajaxDeleteFile");
            $.ajax({
                type: "post",
                url: url,
                dataType: "json",
    ...
    SavaFileToDB: function (fileName, relativePath) {
        var url = $Url.BuildProductUrl("/employee/knowledgeAttachment/ajaxSaveFilePath");
    ...
    GetFileList: function () {
        var url = $Url.BuildProductUrl("/employee/knowledgeAttachment/ajaxListFile");
        $.ajax({
            type: "post",
            url: url,
            dataType: "json",
