var ManagePage = {
    ...
    GetInfo: function (id) {
        var url = $Url.BuildWorkFlowUrl("/workFlow/auditTask/ajaxGetAuditTask");
        $.ajax({
            type: "post",
            url: url,
            dataType: "json",
...
$(function () {
    $("#back").click(function () {
        window.location.href = $Url.BuildWorkFlowUrl("/workFlow/auditTask/list");
    });
    ...
    var auditTaskAdd = $("#auditTaskAdd").Validform({
        ...
        callback: function (form) {
            ...
            var url = $Url.BuildWorkFlowUrl("/workFlow/auditTask/ajaxEditAuditTask");
            var oper = "add";
            if (Number(PageVar.ID) != 0)
                oper = "edit";
            ...
            $.ajax({
                ...
                success: function (data, textStatus) {
                    if (data.errCode == "0000") {
                        window.location.href = $Url.BuildWorkFlowUrl("/workFlow/auditTask/edit?id=" + data.errDesc);
                    } else {
                        $("#msg").text(data.errDesc);
                    }
                },
