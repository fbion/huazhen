var ManagePage = {
    ...
    GetInfo: function (id) {
        var url = $Url.BuildWorkFlowUrl("/workFlow/actReProDef/ajaxGetactReProcDef");
        $.ajax({
            type: "post",
            url: url,
            dataType: "json",
...
$(function () {
    $("#back").click(function () {
        window.location.href = $Url.BuildWorkFlowUrl("/workFlow/aCTREPROCDEF/list");
    });
    ...
    var aCTREPROCDEFAdd = $("#aCTREPROCDEFAdd").Validform({
        ...
        callback: function (form) {
            ...
            var url = $Url.BuildWorkFlowUrl("/workFlow/aCTREPROCDEF/ajaxEditACTREPROCDEF");
            var oper = "add";
            if (Number(PageVar.ID) != 0)
                oper = "edit";
            ...
            var info = {
				...
				dGRMRESOURCENAME: $("#dGRMRESOURCENAME").val(),
				dESCRIPTION: $("#dESCRIPTION").val(),
				hASSTARTFORMKEY: $("#hASSTARTFORMKEY").val(),
            ...
            $.ajax({
                ...
                success: function (data, textStatus) {
                    if (data.errCode == "0000") {
                        window.location.href = $Url.BuildWorkFlowUrl("/workFlow/aCTREPROCDEF/edit?id=" + data.errDesc);
                    } else {
                        $("#msg").text(data.errDesc);
                    }
                },
