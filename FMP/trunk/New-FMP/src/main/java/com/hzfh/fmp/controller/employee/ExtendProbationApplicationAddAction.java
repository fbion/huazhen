...
public class ExtendProbationApplicationAddAction extends CommonAction {
    ...
    public String execute() throws Exception {
        this.setPageAlias(PageAlias.extendProbationApplicationAdd);
	...
	private void initShowEditButton() {
    	...
    	ExtendProbationApplication tApplication = new ExtendProbationApplication();
    	tApplication=ExtendProbationApplicationModel.getInfo(this.id);
...
//    	}
        if (this.getPagePermissionView() != null && this.getPagePermissionView().isEdit() && StringHelper.isNullOrEmpty(this.activitiNo)) {
            this.showEditButton = true;
            this.showSubmitButton = true;
        }else if(!StringHelper.isNullOrEmpty(this.activitiNo) 
    			&& ActRuTaskModel.getInfoByAciIdAndUserId(this.activitiNo, userId) != null && this.id==0){
        	this.showEditButton = true;
            this.showSubmitButton = true;
        } else {
