...
public class RepaymentNotifyAction extends NotifyAction<RepaymentNotify> {
    ...
    public String execute() throws Exception {
        ...
        if (Integer.valueOf(this.getNotify().getCode()) == StatusCode.SUCCESS) {
            PaymentRefund paymentRefund = PaymentRefundModel.getInfo(Integer.valueOf(this.getNotify().getRequestNo()));
            PaymentMoneyFreeze paymentMoneyFree = PaymentMoneyFreezeModel.getInfoBySnAndType(this.getNotify().getRequestNo(), PaymentType.FREEZETYPE_REPAYMENT);
            if (paymentMoneyFree == null) {
                try {
                    PaymentMoneyFreeze paymentMoneyFreeze = new PaymentMoneyFreeze();
                    paymentMoneyFreeze.setAccountType(PaymentType.P2PCUSTOMER_COMPANY);
                ...
                try {
                    PaymentAccountInformationModel.updateMoneyWithDarw(-paymentRefund.getPayMoney(), paymentRefund.getPayerNo());
                    PaymentAccountInformationModel.updateMoneyFreeze(paymentRefund.getPayMoney(), paymentRefund.getPayerNo());
                } catch (Exception e) {
                    logger.error("更新payment_account_information（用户账户信息表）里的金额出现异常", e);
                }

                try {
                    PaymentRefundModel.updateStatusById(Integer.valueOf(this.getNotify().getRequestNo()), StatusType.CHECKPAYMENT);
                } catch (Exception e) {
    ...
    public String confirmTransaction() {
        ...
        if (Integer.valueOf(this.getNotify().getCode()) == StatusCode.SUCCESS) {
            ...
            if (payMoneyChange == null) {
                try {
                    PaymentAccountInformation paymentAccountInformation = PaymentAccountInformationModel.getInfoByCustomerNo(paymentRefund.getP2pCustomerNo());
                    PaymentMoneyChange paymentMoneyChange = new PaymentMoneyChange();
                    paymentMoneyChange.setP2pCustomerNo(paymentRefund.getP2pCustomerNo());
                    paymentMoneyChange.setP2pCustomerName(paymentRefund.getP2pCustomerName());
                    ...
                    paymentMoneyChange.setMoneyAmountLater(paymentAccountInformation.getMoneyAmount() - paymentRefund.getPayMoney());
                    paymentMoneyChange.setMoneyWithdraw(paymentRefund.getPayMoney());
                ...
                try {
                    //更新企业账户
                    PaymentAccountInformationModel.updateMoneyAmount(-paymentRefund.getPayMoney(), paymentRefund.getPayerNo());
                    PaymentAccountInformationModel.updateMoneyFreeze(-paymentRefund.getPayMoney(), paymentRefund.getPayerNo());
                    //更新个人账户
                    PaymentAccountInformationModel.updateMoneyAmount(paymentRefund.getPayMoney(), paymentRefund.getP2pCustomerNo());
                    PaymentAccountInformationModel.updateMoneyWithDarw(paymentRefund.getPayMoney(), paymentRefund.getP2pCustomerNo());
                } catch (Exception e) {
                    logger.error("更新payment_account_information（用户账户信息表）里的金额出现异常", e);
                }

                try {
                    PaymentRefundModel.updateStatusById(paymentRefund.getId(), StatusType.FINSHPAYMENT);
                } catch (Exception e) {
    ...
    public String cancelTransaction() {
        ...
        if (Integer.valueOf(this.getNotify().getCode()) == StatusCode.SUCCESS) {
            ...
            if (paymentMoneyFree == null) {
                try {
                    PaymentMoneyFreeze paymentMoneyFreeze = new PaymentMoneyFreeze();
                    paymentMoneyFreeze.setMoneyFreeze(paymentRefund.getPayMoney());
                    paymentMoneyFreeze.setRefSn(requestNo);
                    paymentMoneyFreeze.setP2pCustomerNo(paymentRefund.getP2pCustomerNo());
                    paymentMoneyFreeze.setP2pCustomerName(paymentRefund.getP2pCustomerName());
                    paymentMoneyFreeze.setState(PaymentType.FREEZESTATE_THRAW);
                ...
                try {
                    PaymentRefundModel.updateStatusById(paymentRefund.getId(), StatusType.CANCLEPAYMENT);
                } catch (Exception e) {
