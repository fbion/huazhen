...
import com.hzfh.fmp.model.common.cache.RedisExpire;
import com.hzfh.fmp.model.common.cache.RedisManager;
import com.hzfh.fmp.model.common.cache.RedisPrefix;
...
public class EnumListCacheModel {
    ...
    public static List<ListItem> getDicDataListByType(int dicTypeNo, boolean isReadCached) {
        ...
        if (isReadCached) {
            String dicData = RedisManager.hget(RedisPrefix.BASEINFO + "dicData", String.valueOf(dicTypeNo));
            if (!StringHelper.isNullOrEmpty(dicData)) {
                itemList = (List<ListItem>) JSONArray.toCollection(JSONArray.fromObject(dicData), ListItem.class);
                return itemList;
            }
        }
        itemList = EnumListModel.getDicDataListByType(dicTypeNo);
        RedisManager.hset(RedisPrefix.BASEINFO + "dicData", String.valueOf(dicTypeNo), JSONArray.fromObject(itemList).toString(), RedisExpire.ONE_MONTH);
        return itemList;
    }
    ...
    public static List<ListItem> getCustomerPersonList(String customerNo, boolean isReadCached) {
        List<ListItem> itemList = new ArrayList<>();
        if (isReadCached) {
            String value = RedisManager.hget(RedisPrefix.CUSTOMER + "customerPersonalList", customerNo);
            if (!StringHelper.isNullOrEmpty(value)) {
                itemList = (List<ListItem>) JSONArray.toCollection(JSONArray.fromObject(value), ListItem.class);
                return itemList;
            }
        }
        itemList = EnumListModel.getCustomerPersontById(Integer.valueOf(customerNo));
        Map<String, String> valueMap = new HashMap<>();
        valueMap.put(customerNo, JSONArray.fromObject(itemList).toString());
        RedisManager.hset(RedisPrefix.CUSTOMER + "customerPersonalList",customerNo, JSONArray.fromObject(itemList).toString(), RedisExpire.HALF_MONTH);
        return itemList;
    }
    ...
    public static List<ListItem> getSuppliers(boolean isReadCached) {
        List<ListItem> itemList = new ArrayList<ListItem>();
        if (isReadCached) {
            itemList = FlushCacheHelper.getCacheForMethod("getSuppliers");
            if (itemList != null && itemList.size() != 0) {
                return itemList;
            }
        }
        itemList = EnumListModel.getSuppliersList();
        FlushCacheHelper.setCacheForMethod("getSuppliers", itemList);
        return itemList;
    }
