...
public class FlushCacheHelper {
	...
	public static void flushMenuTree(int roleNo) {
		int FIRSTLEVELMENU = 2;
		int SECONDLEVELMENU = 3;
		int THREELEVELMENU = 4;
		List<Element> menuList = MenuModel.getUserElementByRoleNoAndLevel(roleNo, 0, String.valueOf(FIRSTLEVELMENU));
		for (Element element : menuList) {
			element.setValue(UrlHelper.buildLoginSiteUrl(element.getValue()));
		}
		CacheManager.set(CachePrefix.EASYUIMENU, String.valueOf(roleNo) + "getEasyUIAccordion", 7 * 24 * 60 * 60, menuList);
		String level = SECONDLEVELMENU +"," +THREELEVELMENU;
		for (Element element:menuList){
			menuList = MenuModel.getUserElementByRoleNoAndLevel(roleNo, element.getId(), level);
			CacheManager.set(CachePrefix.TREELIST, String.valueOf(roleNo)+element.getId() + "menuList", 7 * 24 * 60 * 60, menuList);
		}
