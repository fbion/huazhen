	...
	$.fn.passwordStrength=function(settings){
		...
		this.each(function(){
			var $this=$(this),
				scores = 0,
				checkingerror=false,
				pstrength = $(this).parents(".validform").find(".passwordStrength");
				
			$this.bind("keyup blur",function(){
				scores = $.fn.passwordStrength.ratepasswd($this.val(),settings);
				scores>=0 && checkingerror==false && (checkingerror=true);
				
				pstrength.find("li").removeClass("selected");
				if(scores < 35 && scores >=0){
				    pstrength.find("li:first").addClass("selected");
				}else if(scores < 60 && scores >=35){
				    pstrength.find("li:lt(2)").addClass("selected");
				}else if(scores >= 60){
				    pstrength.find("li:lt(3)").addClass("selected");
				}
